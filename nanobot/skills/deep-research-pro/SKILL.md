...
version: 1.0.0
description: "Multi-source deep research agent. Searches the web, synthesizes findings, and delivers cited reports. Use when users need comprehensive research on any topic, want to gather information from multiple sources, or need a well-structured report with citations."
...
For EACH sub-question, call `web_search` tool:
...
For the most promising URLs, call `web_fetch` tool:
...
Save the full report by calling `write_file` tool:
